...
async def handle_watch(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg += f"<br>🧭 Сводка:<br>{generate_summary(stats_by_range)}"
    ...
    if 'Покупатели' in main_range["side"]:
        ...
        msg += "<br><b>💡 Торговая идея:</b>"
        msg += f"<br><b>✅ Сценарий:</b> Лонг от {support_value - 25:.0f}–{support_value:.0f} $"
        msg += f"<br>⛔ Стоп-лосс: ниже {sl:.0f} $"
        msg += f"<br><b>🎯 Цель:</b> {tp:.0f}–{tp + 500:.0f} $"
        msg += "<br>🔎 Фильтр: свеча/объём 1–5м"


    # Безопасная генерация торговой идеи
    main_range = stats_by_range["±0.2%"]
    if "Покупатели" in main_range["side"]:
        support_price = main_range["support"].split(" $")[0]
        support_value = Decimal(support_price)
        ...
        tp = support_value * Decimal("1.005")
        msg += "\n\n💡 Торговая идея:"
        msg += f"\n<b>✅ Сценарий:</b> Лонг от {support_value - 25:.0f}–{support_value:.0f} $"
        msg += f"\n⛔ Стоп-лосс: ниже {sl:.0f} $"
        msg += f"\n<b>🎯 Цель:</b> {tp:.0f}–{tp + 500:.0f} $"
        msg += "\n🔎 Фильтр: свеча/объём 1–5м"
    ...
    if "Покупатели" in main_range["side"]:
        ...
        tp = support_value * Decimal("1.005")
        msg += "\n\n💡 <b>Торговая идея:</b>"
        msg += f"\n<b>✅ Сценарий:</b> Лонг от поддержки {support_value - 25:.0f}–{support_value:.0f} $"
        msg += f"\n⛔ Стоп-лосс: Ниже поддержки → {sl:.0f} $"
        msg += f"\n<b>🎯 Цель:</b> {tp:.0f}–{tp + 500:.0f} $ (захват ликвидности)"
    ...
    if "Покупатели" in main_range["side"]:
        ...
        tp = support_value * Decimal("1.005")
        msg += f"\n\n💡 <b>Торговая идея:</b>\n"
        msg += f"<b>✅ Сценарий:</b> Лонг от поддержки {support_value - 25:.0f}–{support_value:.0f} $\n"
        msg += f"⛔ Стоп-лосс: Ниже поддержки → {sl:.0f} $\n"
        msg += f"<b>🎯 Цель:</b> {tp:.0f}–{tp + 500:.0f} $ (захват ликвидности)\n"
...
Параметр         | Значение
------------------|-------------------------------
<b>✅ Сценарий</b>       | <b>Лонг от поддержки {support_value - 25:.0f}–{support_value:.0f} $</b>
⛔ Стоп-лосс      | Ниже поддержки → {sl:.0f} $
<b>🎯 Цель</b>           | <b>{tp:.0f}–{tp + 500:.0f} $</b> (захват ликвидности)
🔎 Доп. фильтр    | Подтверждение объёмом / свечой 1–5м
</pre>"""
    ...
    msg += (
        ...
        f"Параметр         | Значение\n"
        f"------------------|-------------------------------\n"
        f"<b>✅ Сценарий</b>       | <b>Лонг от поддержки {support_value - 25:.0f}–{support_value:.0f} $</b>\n"
        f"⛔ Стоп-лосс      | Ниже поддержки → {sl:.0f} $\n"
        f"<b>🎯 Цель</b>           | <b>{tp:.0f}–{tp + 500:.0f} $</b> (захват ликвидности)\n"
