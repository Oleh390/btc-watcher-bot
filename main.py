...
def calculate_stats(depth, price, pct):
    ...
    asks = [x for x in depth["asks"] if Decimal(x[0]) <= upper]
    bids = [x for x in depth["bids"] if Decimal(x[0]) >= lower]
...
async def handle_watch(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Торговая идея (если доминируют покупатели)
