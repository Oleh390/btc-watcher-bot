...
def calculate_stats(data, price, depth_pct):
    ...
    buy_volume = sum(p * q for p, q in bids if lower <= p <= upper)
    sell_volume = sum(p * q for p, q in asks if lower <= p <= upper)

    buy_btc = sum(q for p, q in bids if lower <= p <= upper)
    sell_btc = sum(q for p, q in asks if lower <= p <= upper)

    top_support = max([p for p, q in bids if lower <= p <= upper], default=Decimal("0"))
    top_resistance = min([p for p, q in asks if lower <= p <= upper], default=Decimal("0"))

    support_btc = sum(q for p, q in bids if p == top_support)
    resistance_btc = sum(q for p, q in asks if p == top_resistance)
...
async def main():
    ...
    application.add_handler(CommandHandler("watch", handle_watch))
